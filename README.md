...
HappyClaw 是一个自托管的多用户本地 AI Agent 系统，当前同时支持 **Claude Code** 和 **Codex** 两条原生 runtime 链路。Claude runtime 通过 [Claude Agent SDK](https://github.com/anthropics/claude-code/tree/main/packages/claude-agent-sdk) 驱动完整的 Claude Code CLI；Codex runtime 直接调用 `codex exec --json`。两者都可以通过飞书、Telegram、QQ 和 Web 界面访问，并共享文件读写、终端操作、浏览器自动化、多轮推理和 MCP 工具生态。

核心设计原则：**不重新实现 Agent 能力，直接复用成熟 runtime**。底层调用的是原生 CLI / SDK 执行链路，而不是 API Wrapper 或 Prompt Chain。HappyClaw 负责的是多用户隔离、会话编排、权限控制、消息路由、持久记忆和运维界面。

### 关键特性

- **双 Runtime 驱动** — 每个会话可固定选择 Claude Code 或 Codex，初始化向导和设置页都支持管理默认 runtime 与 provider
- **双执行模式** — Claude / Codex 都可运行在宿主机模式或容器模式；admin 主工作区默认宿主机，member 主工作区默认容器
...
系统把 **runtime** 和 **execution mode** 拆成两个独立维度：

- **Runtime 维度** — Claude 会话走 Claude Agent SDK + Claude Code CLI；Codex 会话走原生 Codex CLI
- **Execution Mode 维度** — 同一个 runtime 可以跑在宿主机或 Docker 容器中
- **Per-user 主工作区** — 每个用户拥有一个固定的主工作区；admin 默认 `main` 宿主机工作区，member 默认 `home-{userId}` 容器工作区
...
- **运行时主记忆文件** — Claude 会话使用 `CLAUDE.md`，Codex 会话使用 `AGENTS.md`
- **用户全局记忆** — `data/groups/user-global/{userId}/`，每个用户独立的全局记忆目录，所有会话可读
- **工作区记忆** — `data/groups/{folder}/{CLAUDE.md|AGENTS.md}`，按 runtime 选择主记忆文件
- **日期记忆** — `data/memory/{folder}/YYYY-MM-DD.md`，存放时效性信息
...
- **用户级 Skills** — 存储在 `data/skills/{userId}/`，运行时会自动链接到对应 session 的 `~/.claude/skills` 或 `~/.codex/skills`
- **宿主机同步** — admin 可把宿主机 `~/.claude/skills/` 同步到用户级 Skill 目录
...
- **至少一种 AI runtime 凭据**
  - Claude：支持 OAuth、setup-token、Anthropic API Key、兼容网关
  - Codex：支持 OpenAI API Key，或直接导入本机 `~/.codex` 配置

- **可选：宿主机 Codex CLI** — 如果你要让宿主机模式工作区运行 Codex，需要本机可执行 `codex` 命令，或在设置页把 Codex command 指向自定义前缀命令
...
> 容器模式的镜像会内置 Claude Code CLI 和 Codex CLI。宿主机模式则使用本机已安装的运行时命令与项目依赖。
...
2. **选择默认 Runtime** — Claude 或 Codex
3. **配置对应 Provider / 凭据**
  - Claude：OAuth、setup-token、API Key 或第三方网关
  - Codex：OpenAI API Key 或导入本机 `~/.codex`
4. **配置 IM 通道**（可选）— 飞书 App ID/Secret、Telegram Bot Token 或 QQ Bot 凭据
5. **开始对话** — 在 Web 聊天页面直接发送消息
...
| 模式 | 说明 | 适用对象 | 前置要求 |
|------|------|---------|---------|
| **宿主机模式** | Agent 直接在宿主机运行，访问本地文件系统；支持 Claude / Codex | admin 主工作区默认模式，也可手动创建宿主机会话 | Claude 依赖已随项目提供；Codex 需本机 `codex` CLI 或自定义 command |
| **容器模式** | Agent 在 Docker 容器中隔离运行，预装 40+ 工具；支持 Claude / Codex | member 主工作区默认模式，也可手动创建容器会话 | Docker + 已构建容器镜像 |
...
| 类别 | 工具 |
|------|------|
| AI / Agent | Claude Code CLI、Codex CLI、Claude Agent SDK、MCP SDK |
...
flowchart TD
    ...
    subgraph 执行层
        ...
        Container["Docker 容器<br/>(agent-runner / codex exec)"]
    end

    subgraph Agent["Agent 运行时"]
        Runtime["原生 Runtime<br/>(Claude Agent SDK / Codex CLI)"]
        MCP["MCP Server<br/>(10 个工具)"]
        Stream["流式事件<br/>(多事件类型)"]
    end

    DB[("SQLite<br/>(WAL 模式)")]
    IPC["IPC 文件通道<br/>(原子读写)"]
    Memory["记忆系统<br/>(CLAUDE.md / AGENTS.md + data/memory/)"]
    ...
    class Runtime,MCP,Stream faas
    class IPC cfg
    class Memory cfg
```

**数据流**：消息从接入层进入主进程，经去重和路由后分发到并发队列。队列根据会话的 runtime 和 execution mode 启动宿主机进程或 Docker 容器。Claude 会话通过 `agent-runner + Claude Agent SDK` 执行，Codex 会话通过 `codex exec --json` 执行。流式事件通过 stdout / JSON 事件回传主进程，再经 WebSocket 广播到 Web 客户端或通过 IM API 回复到飞书/Telegram。MCP Server 通过基于文件的 IPC 通道提供 10 个工具，实现 Agent 与主进程的双向通信。
...
│   ├── agent-runtime.ts          #   runtime 元数据（Claude / Codex）
│   ├── container-runner.ts       #   Docker / 宿主机进程管理
│   ├── codex-config.ts           #   Codex provider 配置读写
...
│   │   └── {CLAUDE.md|AGENTS.md} #     按 runtime 选择的主记忆文件
│   ├── groups/user-global/{id}/  #   用户全局记忆目录
│   │   └── {CLAUDE.md|AGENTS.md} #     用户级主记忆文件
│   ├── sessions/{folder}/        #   运行时会话持久化
│   │   └── { .claude | .codex }/ #     对应 runtime 的状态目录
│   ├── ipc/{folder}/             #   IPC 通道（input / messages / tasks）
│   ├── env/{folder}/env          #   容器环境变量文件
│   ├── memory/{folder}/          #   日期记忆（YYYY-MM-DD.md）
│   ├── skills/{userId}/          #   用户级 Skills 存储
│   └── config/                   #   加密配置文件
│
└── Makefile                      # 常用命令
```
